...
if (mode === "--check-baseline" || mode === "--write-baseline") {
  ...
  const current = Object.fromEntries(
    [...perFile].map(([file, findings]) => [file, findings.length]).sort(),
  );
  if (mode === "--write-baseline") {
    writeFileSync(BASELINE_PATH, JSON.stringify(current, null, 2) + "\n");
    console.log(
      `dash-baseline.json written: ${Object.keys(current).length} files, ${Object.values(current).reduce((a, b) => a + b, 0)} dashes`,
    );
    process.exit(0);
  }
  const baseline = existsSync(BASELINE_PATH) ? JSON.parse(readFileSync(BASELINE_PATH, "utf8")) : {};
...
console.error(
  "usage: check-dashes.mjs --files <f>... | --check-baseline | --write-baseline | --text <s>",
);
process.exit(2);
